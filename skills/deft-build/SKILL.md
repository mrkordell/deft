...
- ! Check for `~/.config/deft/USER.md` (or `$DEFT_USER_PATH`) before doing anything
- ! If USER.md is missing → redirect to `/deft-setup` Phase 1; ⊗ proceed without it
- ! Check for `./SPECIFICATION.md` — required to build
- ! If SPECIFICATION.md is missing → redirect to `/deft-setup` Phase 3; ⊗ proceed without it
- ~ Check for `./PROJECT.md` — recommended but not blocking

## File Reading

- ! Read in order, lazy load:
  1. `~/.config/deft/USER.md` — user preferences (highest precedence, required)
  2. `./SPECIFICATION.md` — what to build (required)
  3. `./PROJECT.md` — project config, tech stack, strategy
